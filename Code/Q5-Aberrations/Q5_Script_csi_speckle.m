%%%
%
% Script to iterate the analysis done in Q5_mod_csi_speckle_algo.m over 
...
Results = 'Results_Q5_speckle_algo0.txt';
ID = fopen(Results,'a+');
...
for i = 1:length(fileList)
    ...
    if contains(Filename, 'tif')
        ...
        for k = 1:length(Mag)-3
            k
            openname = strcat('Q5_Speckle_Mag_', num2str(Mag(k)), '.mat');
            load(openname);
            
            % Normalise illumination accross algorithm (reference CSI_PSF)
            output_img_H = output_img_H * csipsf_total / csicore_total;
            output_img_H_abn = output_img_H_abn * csipsf_total / csicore_total;
            
            fprintf([filenameSaving '\n']);
            [ imageRec, R ] = Q5_mod_csi_speckle_algo(obj, output_img_H, output_img_H_abn , N, 2401, 0, algo);
            fprintf('%10.8f\n', R);
