...
Results = 'Results_Q6_algo0_focus_numbers.txt';
ID = fopen(Results,'a+');
    ...
    if contains(Filename, 'tif')
        ...
        obj = double(imread(Filename));
            for k = 1:length(noise)
                ...
                %Saving the images
                s = strcat('Output/Q6_algo0_focus_numbers/',filenameSaving(1:end-4),'_algo',num2str(algo),'_noise',num2str(k),'_CSIPSF.tif');
