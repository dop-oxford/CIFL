function [result, iter] = RLTV_SVdeconv(image, PSF, fCoeffMaps, modes, lambda, iteration, obj)
...
ratio = image;
for j = 1:modes
    PSFi(:,:,j) = PSF1;%double(imread(PSF, 'index', j));
...
for i=1:maxIteration
   ...
   % Save iteration number if automatic end is to be used.
   iter = i;
%    R1test = corrcoef(obj,abs(fn));
%    R2test = corrcoef(image,abs(fn));
%    fprintf(['\t%10.0f\t%10.8f\t%10.8f\n'], i, R1test(2,1), R2test(2,1))
