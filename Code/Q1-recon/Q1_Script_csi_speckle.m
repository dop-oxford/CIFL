...
%algo = 1;
noise = [0, 1e-17, 1e-16, 1e-15, 1e-14, 1e-13, 1e-12, 1e-11, 1e-10, 1e-9, 1e-8, 1e-7, 1e-6 ];
algo = 0;
...
Results = 'Results_Q1_algo1_csicore_norm_sample.txt';
ID = fopen(Results,'a+');
    ...
    if contains(Filename, 'tif')
        ...
        obj = double(imread(Filename));
            for k = 1:length(noise)
                ...
                %Saving the images
                s = strcat('Output/CSIcorenorm-algo0/',filenameSaving(1:end-4),'_algo',num2str(algo),'_noise',num2str(k),'.tif');
